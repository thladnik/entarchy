...
class SQLiteBackend(Backend):
    ...
    def sql_engine(self) -> sqlalchemy.Engine:
        if not hasattr(self, '_sql_engine') or self._sql_engine is None:
            ...
            self._sql_engine = create_engine(f'sqlite:///{root_path}/{self.dbname}', echo=self.debug,
                                             # Wait for concurrent writers instead of
    ...
    def delete(self, confirm: bool = False):
        ...
        # Release file handles first, otherwise the file cannot be removed on Windows
        self.close()
            ...
            with self.sql_session as session:
                ...
                update_attr_data = {
                    ...
                    **{f'value_{dt}': None for dt in list(set(_dtypes) - {data_type_str})},
                    ...
                    'float_is_nan': insert_stmt.excluded.float_is_nan,
                    'float_is_inf': insert_stmt.excluded.float_is_inf,
                }

                # For floats, value_int carries the sign of inf values and must
                #  override the generic value-column reset above
                if data_type_str == 'float':
                    update_attr_data['value_int'] = insert_stmt.excluded.value_int
